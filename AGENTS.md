...
rtcompare is a small, focused Go library for statistically robust runtime/memory measurement comparisons and lightweight benchmarking — timing sample collection, bootstrap-based distribution comparison, a deterministic PRNG, and small statistics helpers. It's a practical alternative to the standard `testing` benchmark harness when reproducible, distribution-aware comparisons and confidence estimates are needed.
...
- All pushes and PRs are checked by: golangci-lint (`linter.yml`), `go vet`, and `go test`.
- Coverage is tracked via a gist-based badge.
- Dependency updates are automated via Renovate.
- The latest release is scanned for vulnerabilities via `grype-me-latest-release.yml`.
- No CodeQL, dependency-review, or Scorecard workflow yet.
...
- `cmd/rtcompare-example/` is a runnable usage example, not a shipped release binary.
